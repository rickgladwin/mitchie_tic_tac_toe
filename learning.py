...
def update_game_history(opponent_name, opponent_char, opponent_game_result, final_blank_weights, seen_board_states_count) -> None:
    conn = create_connection('sqlite/' + opponent_name + '_' + opponent_char + '.db')
    conn.cursor().execute("INSERT INTO game_history (blank_weights, seen_states_count, result, created_at) VALUES ('" + final_blank_weights + "', '" + str(seen_board_states_count) + "', '" + opponent_game_result + "', '" + str(datetime.now()) + "')")
    conn.commit()
    conn.close()
